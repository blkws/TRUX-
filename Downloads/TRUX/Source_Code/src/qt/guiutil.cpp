...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("globaltruckexchange"))
        return false;
...
{
    // Convert globaltruckexchange:// to globaltruckexchange:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("globaltruckexchange://"))
    {
        uri.replace(0, 12, "globaltruckexchange:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GlobalTruckExchange.lnk";
}
...
{
    return GetAutostartDir() / "globaltruckexchange.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=GlobalTruckExchange\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GlobalTruckExchange-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  globaltruckexchange-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("GlobalTruckExchange-Qt"));
